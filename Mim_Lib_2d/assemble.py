...
def assemble(M2bass, Gi_, G_j, mode='add', assembler='advanced', info=None):
    """
    The caller
    #INPUTS----------
        #ESSENTIAL:
        ...
        #OPTIONAL:
            ...
            assembler :: (strings, default: 'advanced') which assembler we use.
            info :: the other information we may need
    #OUTPUTS---------
        Assembled :: (matrix) the assembled matrix.
    """
    if Gi_.__class__.__name__ != 'ndarray':
        Gi_ = Gi_.dof_map
    if G_j.__class__.__name__ != 'ndarray':
        G_j = G_j.dof_map
    ...
    assert (row == rowGi_) and (colume == rowG_j) and \
           (columnGi_ == columnG_j) and (columnG_j == num_elements), "data structure wrong......"
...
def _assembler_baseV00(M2bass, Gi_, G_j, mode):
    ...
    hmgeoiti_ = int(np.max(Gi_) + 1); hmgeoit_j = int(np.max(G_j) + 1)

    szGi_ = np.shape(Gi_); szG_j = np.shape(G_j)
    rowGi_ = szGi_[0]; rowG_j = szG_j[0]
    ...
    else: raise Exception('Mode wrong: add, replace or average......')
    return assembled.tocsc()
...
def assemble_array(array, dof_map_rows, dof_map_cols, assemble=False, order='F'):
    ...
    ----------
    array_local : ndarray
        3d array containing the inner product of basis functions element-wise.
    ...
    return   coo_matrix((data, (row_idx, column_idx))
    ...
    inner_sparse = sparse.coo_matrix(
