...
import numpy as np
import functionals as functionals
...
class BasicMesh(ABC):
    ...
    def __init__(self, dim, R, elements_layout, info, name, pre_No):
        ...
        if isinstance(elements_layout, int):
            ...
            assert np.shape(elements_layout) == (2,), \
                " <MESH> : elements_layout tuple shape wrong, now is {}".format(np.shape(elements_layout))
        ...
        self._elements_corner_points_numbering = None
    ...
    # %% elements's corner point numbering
    @property
    def elements_corner_points_numbering(self):
        """
        #SUMMARY: return 2-d array
                    axis [0] -> # of element
                    axis [1] -> [SW, SE, NW, SE] numbering of element corner points
        """
        if self._elements_corner_points_numbering is not None: 
            return self._elements_corner_points_numbering
        
        nx, ny = self.elements_layout
        
        stn = np.shape(self.R.regions_flat)[0]  # start numbering point
        M = -1 * np.ones(shape=(self.n_x+1, self.n_y+1, self.R.num_regions))
        
        regions_connections_details = self.R.regions_connections_detials
        regions_dof_map = self.R.regions_dof_map
        
        DictNum = {'0': 'S', '1': 'N', '2': 'W', '3': 'E'}
        DictStr = {'S': 0, 'N': 1, 'W': 2, 'E': 3}
        
        for k in range(self.R.num_regions):
            M[0, 0, k] = regions_dof_map[k, 0]  # south-west corner point
            M[-1, 0, k] = regions_dof_map[k, 1]  # south-east corner point
            M[0, -1, k] = regions_dof_map[k, 2]  # north-west corner point
            M[-1, -1, k] = regions_dof_map[k, 3]  # north-east corner point
            
            for i, connection in enumerate(regions_connections_details[k]):
                if connection[0] != '+' and connection[0] != '-':  # this region is on the boundary
                    if i == 0:
                        M[1:-1, 0, k] = range(stn, stn+nx-1)
                        stn += nx-1
                    elif i == 1:
                        M[1:-1, -1, k] = range(stn, stn+nx-1)
                        stn += nx-1
                    elif i == 2:
                        M[0, 1:-1, k] = range(stn, stn+ny-1)
                        stn += ny-1
                    elif i == 3:
                        M[-1, 1:-1, k] = range(stn, stn+ny-1)
                        stn += ny-1
                    else:
                        raise Exception("Error!")
                        
                else:  # this region boundary is not on the domain boundary
                    m, t, tp = connection.split(' ')
                    assert regions_connections_details[int(t)][DictStr[tp]] == \
                        m + ' ' + str(k) + ' ' + DictNum[str(i)]
                    
                    if int(t) > k:  # this boundary is not numbered yet
                        if i == 0:
                            M[1:-1, 0, k] = range(stn, stn+nx-1)
                            stn += nx-1
                        elif i == 1:
                    ...
                    elif int(t) < k:  # this boundary is numbered somehow.
                        if tp == 'S':
                            tM = M[1:-1, 0, int(t)]
                        elif tp == 'N':
                            tM = M[1:-1, -1, int(t)]
            ...
            M[1:-1, 1:-1, k] = np.array([stn + int(i) for i in range((nx-1)*(ny-1))]
                                        ).reshape((nx-1, ny-1), order='F')
            stn += (nx-1) * (ny-1)

        self._elements_corner_points_numbering = -1 * np.ones((self.num_elements, 4))
        for i in range(self.R.num_regions):
            for j in range(ny):
                for k in range(nx):
                    m = j + k*ny + i*nx*ny
                    self._elements_corner_points_numbering[m, 0] = M[k, j, i]  # south-west point
                    self._elements_corner_points_numbering[m, 1] = M[k+1, j, i]  # south-east point
                    self._elements_corner_points_numbering[m, 2] = M[k, j+1, i]  # north-west point
                    self._elements_corner_points_numbering[m, 3] = M[k+1, j+1, i]  # north-east point
    ...
    def elements_map_r_local(self):
        if self._elements_map_r_local is not None:
...
#                the_location = str(k) + ' '
                for i in range(self.n_x):
                    ...
                    if i == 0:  # left element
                        ...
                        _elements_map_s = \
                            (self._fec(the_location+'S', i, 0), self._fec(the_location+'N', i, 0), self._fec(the_location+'W', i, 0), s+1)
                        ...
                        _elements_map_s = \
                            (self._fec(the_location+'S', i, 0), self._fec(the_location+'N', i, 0), s-1, self._fec(the_location+'E', i, 0))
                    else:
                        raise Exception(' <MESH> : Never reach here!!!!!')
            ...
            for k in tqdm(range(self.R.num_regions)):
                ...
                for j in range(self.n_y):
                    ...
                    if j == 0:  # bottom element
                        ...
                        _elements_map_s = \
                            (self._fec(the_location+'S', 0, j), s+1, self._fec(the_location+'W', 0, j), self._fec(the_location + 'E', 0, j))
                        ...
                        _elements_map_s = \
                            (s-1, self._fec(the_location+'N', 0, j), self._fec(the_location+'W', 0, j), self._fec(the_location+'E', 0, j))
                    else:
                        raise Exception(' <MESH> : Never reach here!!!!!')
                ...
                for i in range(self.n_x):
                    for j in range(self.n_y):
                        ...
                        if i == 0 and j == 0:  # south_west corner element
                            ...
                            _elements_map_s = (self._fec(the_location+'S', i, j), s+1, self._fec(the_location+'W', i, j), s+self.n_y)
                        elif i == 0 and (0 < j < self.n_y-1):  # west elements
                            the_location = 'W ' + str(k) + ' '
                            _elements_map_s = (s-1, s+1, self._fec(the_location+'W', i, j), s+self.n_y)
                        elif i == 0 and j == self.n_y-1:  # north_west element
                            the_location = 'NW ' + str(k) + ' '
                            _elements_map_s = (s-1, self._fec(the_location+'N', i, j), self._fec(the_location+'W', i, j), s+self.n_y)
                        elif (0 < i < self.n_x-1) and j == 0:  # south elements
                            ...
                            _elements_map_s = (self._fec(the_location+'S', i, j), s+1, s-self.n_y, self._fec(the_location+'E', i, j))
                        elif i == self.n_x-1 and (0 < j < self.n_y-1):  # east elements
                            the_location = 'E ' + str(k) + ' '
                            _elements_map_s = (s-1, s+1, s-self.n_y, self._fec(the_location+'E', i, j))
                        elif i == self.n_x-1 and j == self.n_y-1:  # north-east elements
                            the_location = 'NE ' + str(k) + ' '
                            _elements_map_s = (s-1, self._fec(the_location+'N', i, j), s-self.n_y, self._fec(the_location+'E', i, j))
                        else:
                            raise Exception(' <MESH> : Never reach here!!!!!')
                        self._elements_map_r += (_elements_map_s,)
        else:
    ...
    def elements_numbering(self):
        ...
        """
        if self._elements_numbering is not None:
    ...
    # %% correct metric term multiple g 
    def g_11_g(self, xi, eta, element=None):
        g_11_result = (self.dx_deta(xi, eta, element) * self.dx_deta(xi, eta, element) +
                       self.dy_deta(xi, eta, element) * self.dy_deta(xi, eta, element)) / self.g(xi, eta, element)
        return g_11_result

    def g_12_g(self, xi, eta, element=None):
        g_12_result = -(self.dy_deta(xi, eta, element) * self.dy_dxi(xi, eta, element) +
                        self.dx_dxi(xi, eta, element) * self.dx_deta(xi, eta, element)) / self.g(xi, eta, element)
        return g_12_result

    def g_22_g(self, xi, eta, element=None):
        g_22_result = (self.dx_dxi(xi, eta, element) * self.dx_dxi(xi, eta, element) +
                       self.dy_dxi(xi, eta, element) * self.dy_dxi(xi, eta, element)) / self.g(xi, eta, element)
        return g_22_result
    ...
    # %% PLOT ITSELF
    def plot_mesh(self, regions=None, elements=None, plot_density=20,
                  internal_mesh_type=('lobatto', (1, 1)),
        ...
        if show_plot is True:
            ...
            # plot the boundary of the region
            for i in range(np.size(regions)):
                plt.plot(x_r[:, :4, i], y_r[:, :4, i], 'b', linewidth=linewidth*1.5)  # REGIONS' EDGES
...
if __name__ == "__main__":
    from meshes_chooser import mesh_No
    mesh = mesh_No(No=0, elements_layout=(3, 3), c=0.3, mesh_type=None)
    a = mesh.elements_corner_points_numbering