...
def general(form):
    ...
    # %% GAUSS_LOBATTO 0 FORM CONTINUOUS NUMBERING
        ...
        """ SUMMARY """
        # %% CRAZYMESH
        if form.mesh.__class__.__name__ == 'CrazyMesh':
            nx, ny = form.mesh.elements_layout
            gpx = px * nx + 1
            gpy = py * ny + 1
            global_matrix = np.array([int(i) for i in range(gpx * gpy)]).reshape(gpy, gpx, order='F')

            numbering_boundaries = ()
            for i in range(form.mesh.R.num_boundaries_sections):
                numbering_boundaries += ([],)
            numbering_boundaries[0].extend(global_matrix[0, :])
            numbering_boundaries[1].extend(global_matrix[-1, :])
            numbering_boundaries[2].extend(global_matrix[:, 0])
            numbering_boundaries[3].extend(global_matrix[:, -1])

            global_numbering = np.zeros(shape=(py + 1, px + 1, num_elements), dtype=np.int64)
            for i in range(nx):
                for j in range(ny):
                    k = j + i * ny
                    global_numbering[:, :, k] = global_matrix[j * py:j * py + py + 1, i * px:i * px + px + 1]

            global_numbering = global_numbering.ravel('F').reshape(num_elements, form.num_basis, order='C')
    ...
    # %% GAUSS_LOBATTO 1 FORM
        ...
        # print("num_boundary========", numbering_boundaries)
        return global_numbering, numbering_boundaries
